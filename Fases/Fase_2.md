...
> 1. **Prepárate primero (sin grabar):** comprueba lo necesario, **léete el procedimiento entero** y **crea la entrada de apuntes de esta fase** en Obsidian: fichero `v3-fase-2-purga-y-preparacion-del-entorno.md` dentro de `00_Apuntes/Trimestre_N/B4_Ubuntu_Nube/`, con la estructura de la Fase 0.1 y **vacía**. Rellenarla es cosa tuya, después.
> 2. **Arranca OBS y PRESÉNTATE:** *"Hola, me llamo [Nombre], 2.º SMR, y en este vídeo voy a explicar la Fase 2 de Boochan V3 — Purga y Preparación del Entorno."* Y **muestra algo que demuestre que eres tú** (tu perfil de GitHub, tu Teams o tu correo `@alu.edu.gva.es`). Di qué vas a hacer.
...
> 5. **Al terminar:** nombra el vídeo `V3 · Fase 2 — Purga y Preparación del Entorno`, súbelo a tu playlist de YouTube **`B4_Ubuntu_Nube`** (No listado) y **copia su enlace**.
...
> Creaste una instancia EC2 con Ubuntu Server en AWS. Está encendido, accesible por SSH, protegido por Security Group. Pero viene "de fábrica" con software innecesario: servicios antiguos, demonios durmiendo, paquetes que consumirán RAM y podrían ser puertas de seguridad.
...
> Todos los comandos de esta fase se ejecutan **dentro de tu instancia EC2 en AWS**, no en tu PC del aula. Abre una terminal y conéctate via SSH como hiciste en la Fase 1:
> ```bash
...
> 1. **Crea la entrada de apuntes** de esta fase (`v3-fase-2-purga-y-preparacion-del-entorno.md`) con su estructura, vacía.
...
> Debemos decirle al servidor quién es. Primero averiguamos su dirección IP en la red interna de la VPC de AWS:
> ```bash
> # Muestra las IPs del servidor
> hostname -I
> ```
> > [!tip] 💡 ¿Qué IP anoto si aparecen varias?
> > El comando puede mostrar dos o tres números. Tu IP privada es la que **NO** es `127.0.0.1` (esa es la de loopback, siempre presente) ni `10.0.0.1` (esa será la de la VPN WireGuard, que aún no existe en esta fase). En la VPC por defecto de AWS suele **empezar por `172.31.`** (por ejemplo `172.31.20.45`). Apunta el número completo; cada instancia recibe uno distinto.
> >
...
> > > Algunos laboratorios usan una VPC con un rango distinto (por ejemplo `10.0.x.x` o `172.30.x.x`). No pasa nada: la regla general es coger **la única IP privada que te muestra `hostname -I`** descartando `127.0.0.1`. Si tienes dudas de cuál es, ejecuta este comando, que muestra directamente la IP privada principal de la instancia:
> > > ```bash
> > > ip -4 addr show $(ip route show default | awk '{print $5}') | grep -oP 'inet \K[\d.]+'
> > > ```
> > > Esa es la IP que debes escribir en `/etc/hosts`, empiece por `172.31.`, `172.30.` o lo que sea.
>
...
> sudo nano /etc/hosts
> ```
> Dentro del archivo verás varias líneas existentes. **Añade la siguiente línea al final del archivo**, sustituyendo `172.31.X.X` por la IP que anotaste:
> ```
> 172.31.X.X  UbuntuServer.BOOCHAN.SPACE  UbuntuServer
> ```
> Guarda y sal (`Ctrl + O`, `Enter`, `Ctrl + X`).
>
...
> | :--- | :--- | :--- |
> | **Entrada de apuntes** | `00_Apuntes/Trimestre_N/B4_Ubuntu_Nube/v3-fase-2-purga-y-preparacion-del-entorno.md` | Estructura completa + **respuestas a las Preguntas Críticas y al 🔬 Reto** + **enlace del vídeo** |
> | **Vídeo** | Playlist `B4_Ubuntu_Nube` (No listado) | Nombrado `V3 · Fase 2 — Purga y Preparación del Entorno`, con presentación, identidad y timestamps |
...
>
> > [!info] 🏷️ Por qué el nombre lleva `V3` delante
