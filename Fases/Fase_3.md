...
> **Requisitos:** 2 GB RAM | WireGuard PC | AWS Console | SSH
...
> 1. **Prepárate primero (sin grabar):** comprueba lo necesario, **léete el procedimiento entero** y **crea la entrada de apuntes de esta fase** en Obsidian: fichero `v3-fase-3-conectividad-vpn-wireguard.md` dentro de `00_Apuntes/Trimestre_N/B4_Ubuntu_Nube/`, con la estructura de la Fase 0.1 y **vacía**. Rellenarla es cosa tuya, después.
> 2. **Arranca OBS y PRESÉNTATE:** *"Hola, me llamo [Nombre], 2.º SMR, y en este vídeo voy a explicar la Fase 3 de Boochan V3 — Conectividad VPN (WireGuard)."* Y **muestra algo que demuestre que eres tú** (tu perfil de GitHub, tu Teams o tu correo `@alu.edu.gva.es`). Di qué vas a hacer.
...
> 5. **Al terminar:** nombra el vídeo `V3 · Fase 3 — Conectividad VPN (WireGuard)`, súbelo a tu playlist de YouTube **`B4_Ubuntu_Nube`** (No listado) y **copia su enlace**.
...
> 1. **Crea la entrada de apuntes** de esta fase (`v3-fase-3-conectividad-vpn-wireguard.md`) con su estructura, vacía.
...
> > El túnel funciona. Ahora es el momento de ejecutar las **dos acciones de seguridad** que dejamos pendientes en la Fase 1: cambiar el puerto SSH a 2222 en el servidor y eliminar la regla del puerto 22 del Security Group de AWS. Encuéntralas al final de la Fase 1 en el apartado "Auditoría de Seguridad".
> >
> > A partir de ese momento, **todas tus conexiones SSH usarán este comando** (con la IP de la VPN, no la IP pública de AWS):
> > ```bash
> > ssh -i ~/boochan-key.pem -p 2222 ubuntu@10.0.0.1
> > ```
...
> 5. 🔬 **Reto práctico:** Con el túnel WireGuard **desactivado** en tu PC, intenta conectarte al servidor por SSH usando la IP pública de AWS (no la `10.0.0.1`). ¿Puedes entrar? ¿Por qué sí o por qué no? Razona tu respuesta mirando las reglas del Security Group que configuraste en esta fase.
...
> | :--- | :--- | :--- |
> | **Entrada de apuntes** | `00_Apuntes/Trimestre_N/B4_Ubuntu_Nube/v3-fase-3-conectividad-vpn-wireguard.md` | Estructura completa + **respuestas a las Preguntas Críticas y al 🔬 Reto** + **enlace del vídeo** |
> | **Vídeo** | Playlist `B4_Ubuntu_Nube` (No listado) | Nombrado `V3 · Fase 3 — Conectividad VPN (WireGuard)`, con presentación, identidad y timestamps |
