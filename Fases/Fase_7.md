...
> 1. **Prepárate primero (sin grabar):** comprueba lo necesario, **léete el procedimiento entero** y **crea la entrada de apuntes de esta fase** en Obsidian: fichero `v3-fase-7-seguridad-avanzada-acls-y-abe.md` dentro de `00_Apuntes/Trimestre_N/B4_Ubuntu_Nube/`, con la estructura de la Fase 0.1 y **vacía**. Rellenarla es cosa tuya, después.
> 2. **Arranca OBS y PRESÉNTATE:** *"Hola, me llamo [Nombre], 2.º SMR, y en este vídeo voy a explicar la Fase 7 de Boochan V3 — Seguridad Avanzada (ACLs y ABE)."* Y **muestra algo que demuestre que eres tú** (tu perfil de GitHub, tu Teams o tu correo `@alu.edu.gva.es`). Di qué vas a hacer.
...
> 5. **Al terminar:** nombra el vídeo `V3 · Fase 7 — Seguridad Avanzada (ACLs y ABE)`, súbelo a tu playlist de YouTube **`B4_Ubuntu_Nube`** (No listado) y **copia su enlace**.
...
> Esta fase trabaja íntegramente dentro del servidor — permisos de ficheros, ACLs y configuración de Samba. No requiere abrir ningún puerto nuevo en AWS: todos los necesarios para el acceso desde Windows (SMB 445, Kerberos, LDAP…) ya están activos desde la Fase 4.
...
> 1. **Crea la entrada de apuntes** de esta fase (`v3-fase-7-seguridad-avanzada-acls-y-abe.md`) con su estructura, vacía.
...
> | :--- | :--- | :--- |
> | **Entrada de apuntes** | `00_Apuntes/Trimestre_N/B4_Ubuntu_Nube/v3-fase-7-seguridad-avanzada-acls-y-abe.md` | Estructura completa + **respuestas a las Preguntas Críticas y al 🔬 Reto** + **enlace del vídeo** |
> | **Vídeo** | Playlist `B4_Ubuntu_Nube` (No listado) | Nombrado `V3 · Fase 7 — Seguridad Avanzada (ACLs y ABE)`, con presentación, identidad y timestamps |
