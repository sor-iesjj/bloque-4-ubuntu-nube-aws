...
> **Requisitos:** 4-8 GB RAM | AWS Console | Git | Samba disponible
...
> 1. **Prepárate primero (sin grabar):** comprueba lo necesario, **léete el procedimiento entero** y **crea la entrada de apuntes de esta fase** en Obsidian: fichero `v3-fase-4-aprovisionamiento-del-dominio-samba-ad-d.md` dentro de `00_Apuntes/Trimestre_N/B4_Ubuntu_Nube/`, con la estructura de la Fase 0.1 y **vacía**. Rellenarla es cosa tuya, después.
> 2. **Arranca OBS y PRESÉNTATE:** *"Hola, me llamo [Nombre], 2.º SMR, y en este vídeo voy a explicar la Fase 4 de Boochan V3 — Aprovisionamiento del Dominio (Samba AD DC)."* Y **muestra algo que demuestre que eres tú** (tu perfil de GitHub, tu Teams o tu correo `@alu.edu.gva.es`). Di qué vas a hacer.
...
> 5. **Al terminar:** nombra el vídeo `V3 · Fase 4 — Aprovisionamiento del Dominio (Samba AD DC)`, súbelo a tu playlist de YouTube **`B4_Ubuntu_Nube`** (No listado) y **copia su enlace**.
...
> [!tip] Hoja de Ruta
> 1. Abrir 13 puertos en Security Group de AWS (Kerberos, DNS, LDAP, SMB, RPC, NTP — todo lo que AD necesita)
...
> 4. Comprobar que el DNS interno apunta a 127.0.0.1 (no a AWS): `cat /etc/resolv.conf`
> 5. Hacer inmutable `/etc/resolv.conf` con `chattr +i` para que AWS no lo rompa en reinicios
...
> 1. **Crea la entrada de apuntes** de esta fase (`v3-fase-4-aprovisionamiento-del-dominio-samba-ad-d.md`) con su estructura, vacía.
...
> 3. ¿Qué pasaría si el atributo de inmutabilidad (`+i`) no estuviera activo en el `resolv.conf` tras reiniciar en AWS?
...
> **Peligro Crítico:** Si el DNS vuelve a apuntar a AWS en lugar de a 127.0.0.1, los ordenadores dirán "No se encuentra el dominio" y nadie podrá iniciar sesión.
...
> | :--- | :--- | :--- |
> | **Entrada de apuntes** | `00_Apuntes/Trimestre_N/B4_Ubuntu_Nube/v3-fase-4-aprovisionamiento-del-dominio-samba-ad-d.md` | Estructura completa + **respuestas a las Preguntas Críticas y al 🔬 Reto** + **enlace del vídeo** |
> | **Vídeo** | Playlist `B4_Ubuntu_Nube` (No listado) | Nombrado `V3 · Fase 4 — Aprovisionamiento del Dominio (Samba AD DC)`, con presentación, identidad y timestamps |
