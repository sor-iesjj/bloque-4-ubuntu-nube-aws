...
> 1. **Prepárate primero (sin grabar):** comprueba lo necesario, **léete el procedimiento entero** y **crea la entrada de apuntes de esta fase** en Obsidian: fichero `v3-fase-5-gestion-de-identidades-usuarios-y-grupos.md` dentro de `00_Apuntes/Trimestre_N/B4_Ubuntu_Nube/`, con la estructura de la Fase 0.1 y **vacía**. Rellenarla es cosa tuya, después.
> 2. **Arranca OBS y PRESÉNTATE:** *"Hola, me llamo [Nombre], 2.º SMR, y en este vídeo voy a explicar la Fase 5 de Boochan V3 — Gestión de Identidades (Usuarios y Grupos)."* Y **muestra algo que demuestre que eres tú** (tu perfil de GitHub, tu Teams o tu correo `@alu.edu.gva.es`). Di qué vas a hacer.
...
> 5. **Al terminar:** nombra el vídeo `V3 · Fase 5 — Gestión de Identidades (Usuarios y Grupos)`, súbelo a tu playlist de YouTube **`B4_Ubuntu_Nube`** (No listado) y **copia su enlace**.
...
> El puerto **445 (SMB)** que necesita esta fase ya fue abierto en la **Fase 4**, junto con el resto de puertos de Active Directory. No tienes que añadir ninguna regla nueva en AWS.
>
> Si al conectarte desde Windows la carpeta no aparece y sospechas que es un problema de puerto, verifica en el Security Group (EC2 → Grupos de seguridad → Reglas de entrada) que la regla `SMB_Files` (puerto 445) existe y está habilitada.
...
> 1. **Crea la entrada de apuntes** de esta fase (`v3-fase-5-gestion-de-identidades-usuarios-y-grupos.md`) con su estructura, vacía.
...
> | :--- | :--- | :--- |
> | **Entrada de apuntes** | `00_Apuntes/Trimestre_N/B4_Ubuntu_Nube/v3-fase-5-gestion-de-identidades-usuarios-y-grupos.md` | Estructura completa + **respuestas a las Preguntas Críticas y al 🔬 Reto** + **enlace del vídeo** |
> | **Vídeo** | Playlist `B4_Ubuntu_Nube` (No listado) | Nombrado `V3 · Fase 5 — Gestión de Identidades (Usuarios y Grupos)`, con presentación, identidad y timestamps |
