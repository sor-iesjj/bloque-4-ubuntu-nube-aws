# 🚨 Guía de Errores y Resolución — Proyecto BoochanV3
...
### Error 1.1 — `Permission denied (publickey)` al conectar por SSH

> [!bug] Cuándo se produce
> Al intentar la primera conexión SSH a la instancia EC2. Es el error más habitual en AWS y casi siempre se debe a una de estas tres causas: (1) usas un usuario equivocado (en Ubuntu de AWS el usuario es `ubuntu`, no `root` ni `boochan`); (2) no indicas la clave con `-i`, o indicas un `.pem` que no es el correcto; (3) el fichero `.pem` tiene permisos demasiado abiertos.

> [!warning] ¿Hay que preocuparse?
> No es grave, pero no podrás entrar al servidor hasta resolverlo. No tiene nada que ver con el Security Group (si fuera un puerto cerrado, el error sería *timeout*, no *Permission denied*).

> [!example] Resolución
> 1. Comprueba que usas el **usuario correcto** y la **clave correcta**:
>    ```bash
>    ssh -i ~/boochan-key.pem ubuntu@TU_IP_ELASTICA
>    ```
> 2. En Mac/Linux, ajusta los permisos del `.pem` (AWS rechaza claves "demasiado visibles"):
>    ```bash
>    chmod 400 ~/boochan-key.pem
>    ```
> 3. Verifica que el `.pem` es exactamente el que descargaste al crear el Key Pair `boochan-key` en la Fase 1. Si lo perdiste, no hay forma de recuperarlo: habría que crear un Key Pair nuevo y asociarlo a la instancia (o acceder por SSM, ver Error 1.2).
> 4. Si el error persiste, añade `-v` al comando (`ssh -v -i ...`) para ver en qué punto del intercambio de claves falla.
...
> [!bug] Cuándo se produce
> Si borras la regla del puerto 22 del Security Group de AWS antes de que la VPN esté funcionando y el servidor esté escuchando en el puerto 2222.
...
> [!example] Resolución — Acceso de rescate sin SSH (AWS)
> En AWS el rescate más fiable es **SSM Session Manager**, que abre una shell en la instancia sin depender de SSH ni de ningún puerto del Security Group:
> 1. Entra en la consola de AWS ([console.aws.amazon.com](https://console.aws.amazon.com)) → **EC2** → **Instancias**.
> 2. Selecciona `UbuntuServer` → botón **Conectar** → pestaña **Session Manager** → **Conectar**.
> 3. Se abre una terminal en el navegador, ya como usuario del sistema (usa `sudo -i` para ser root).
> 4. Vuelve al **Security Group** (EC2 → Grupos de seguridad → Reglas de entrada) y añade de nuevo la regla del puerto 22 (TCP, Origen `Anywhere-IPv4`).
> 5. Ya puedes reconectarte por SSH normalmente.
>
> > [!note] Alternativa: EC2 Serial Console
> > Si SSM no estuviera disponible, la **EC2 Serial Console** también sirve (EC2 → Conectar → *EC2 serial console*), pero **requiere** instancia tipo Nitro (t3 lo es) y haber fijado previamente una contraseña al usuario `ubuntu` (`sudo passwd ubuntu`), ya que por defecto no tiene. Por eso SSM es la vía recomendada.
>
...
> [!bug] Cuándo se produce
> Al llegar a fases avanzadas sin haber realizado la auditoría del final de la Fase 1: cambiar el puerto en `/etc/ssh/sshd_config` y eliminar la regla del puerto 22 del Security Group.
...
>    ```
> 6. Si el segundo terminal conecta correctamente, ya puedes eliminar la regla del puerto 22 en el Security Group de AWS.
...
> [!bug] Cuándo se produce
> No se detecta en la Fase 2. El síntoma aparece en la **Fase 4**, cuando `cat /etc/resolv.conf` muestra una IP del DNS de AWS en lugar de `127.0.0.1`.
...
> [!bug] Cuándo se produce
> Cuando se editó `/etc/hosts` para añadir el FQDN pero no se editó `/etc/hostname`. En AWS, el nombre del servidor puede restablecerse al valor original tras un reinicio.
...
> **1. ¿Está el puerto 51820 UDP abierto en el Security Group de AWS?**
> Revisa el Security Group. Debe haber una regla con puerto `51820`, protocolo **UDP** (no TCP) y acción Permitir.
>
...
> [!bug] Cuándo se produce
> Al ejecutar el comando de descarga del repositorio sin sustituir el texto `https://github.com/sor-iesjj/bloque-4-ubuntu-nube-aws` por la URL real. Git devuelve un error inmediato.
...
### Error 4.3 — `resolv.conf` muestra una IP del DNS de AWS en lugar de `127.0.0.1`

> [!bug] Cuándo se produce
> Tras ejecutar el script, al verificar el DNS con `cat /etc/resolv.conf`. El fichero muestra una IP del DNS de la VPC de AWS (por ejemplo `172.31.0.2` o `169.254.169.253`) en lugar de `nameserver 127.0.0.1`.
...
> [!example] Resolución — Acceso de rescate sin SSH (AWS)
> Como la red no responde, entramos por una vía que no depende de SSH. En AWS la más fiable es **SSM Session Manager**:
> 1. Entra en la consola de AWS ([console.aws.amazon.com](https://console.aws.amazon.com)) → **EC2** → **Instancias**.
> 2. Selecciona `UbuntuServer` → **Conectar** → pestaña **Session Manager** → **Conectar**. (Si SSM no estuviera disponible, usa la **EC2 Serial Console**; recuerda que exige instancia Nitro y contraseña previa en el usuario `ubuntu`.)
> 3. Ya dentro de la terminal del navegador, pasa a root con `sudo -i`.
...
>    ```
> 3. Si aún no hay espacio suficiente (se necesitan al menos 12 GB libres), consulta con tu profesor para ampliar el volumen EBS desde la consola de AWS (EC2 → Volúmenes → Modificar volumen, y luego `sudo growpart` + `sudo resize2fs` dentro del servidor), o reduce el tamaño de los discos cambiando `count=5120` por `count=2048` en los comandos `dd` (generará discos de 2 GB en lugar de 5 GB).
...
*Proyecto BoochanV3 — Curso 2025/2026*
